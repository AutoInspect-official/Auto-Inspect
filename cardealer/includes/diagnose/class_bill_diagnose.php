...
add_action("plugins_loaded", "cardealer_bill_hooking_diagnose");
// end >>>>>>>>>>>>>>>>>>>>>>>>>
*/
...
If you need to dive deeper, install the free plugin WPTools, which provides more in-depth insights.',
        ...
        $screen->add_help_tab([
            ...
            'title'   => esc_attr__('Memory & Error Monitoring', 'cardealer'), // Título da aba
            'content' => '<p>' . esc_attr__('Welcome to plugin Insights!', 'cardealer') . '</p>
                          <p>' . $hmessage . '</p>',
        ]);
    }
}
...
class cardealer_Bill_Diagnose
{
    ...
    public function show_dismissible_notification()
    {
        ...
        $message = esc_attr__("Our plugin", 'cardealer');
        $message .= ' (' . $this->plugin_slug . ') ';
        $message .= esc_attr__("cannot function properly because your WordPress Memory Limit is too low. Your site will experience serious issues, even if you deactivate our plugin.", 'cardealer');
        $message .=
            '<a href="' .
            esc_url($this->notification_url) .
            '">' .
            " " .
            esc_attr__("Learn more", 'cardealer') .
            "</a>";
    ...
    public function site_health_tab_content($tab)
    {
        ...
        <div class="wrap health-check-body, privacy-settings-body">
            <p style="border: 1px solid #000; padding: 10px;">
                <strong>
                    <?php
                    echo esc_attr__("Displaying the latest recurring errors (Javascript Included) from your error log file and eventually alert about low WordPress memory limit is a courtesy of plugin", 'cardealer');
                    echo ': ' . esc_attr($this->global_plugin_slug) . '. ';
                    echo esc_attr__("Disabling our plugin does not stop the errors from occurring; it simply means you will no longer be notified here that they are happening, but they can still harm your site.", 'cardealer');
                    echo '<br>';
                    echo esc_attr__("Click the help button in the top right or go directly to the AI chat box below for more specific information on the issues listed.", 'cardealer');
                    ?>
                </strong>
            </p>
            <!-- chat -->
            <div id="chat-box">
                <div id="chat-header">
                    <h2><?php echo esc_attr__("Artificial Intelligence Support Chat for Issues and Solutions", "cardealer"); ?></h2>
                </div>
                ...
                <form id="chat-form">
                    <div id="input-group">
                        <input type="text" id="chat-input" placeholder="<?php echo esc_attr__('Enter your message...', 'cardealer'); ?>" />
                        <button type="submit"><?php echo esc_attr__('Send', 'cardealer'); ?></button>
                    </div>
                    <div id="action-instruction" style="text-align: center; margin-top: 10px;">
                        <span><?php echo esc_attr__("Enter a message and click 'Send', or just click 'Auto Checkup' to analyze the site's error log.", 'cardealer'); ?></span>
                    </div>
                    <div class="auto-checkup-container" style="text-align: center; margin-top: 10px;">
                        <button type="button" id="auto-checkup">
                            ...
                            <?php echo esc_attr__('Auto Checkup', 'cardealer'); ?>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <h3 style="color: red;">
                <?php
                echo esc_attr__("Potential Problems", 'cardealer');
                ?>
            </h3>
            <?php
            $memory = $this->global_variable_memory;
            ...
            if ($memory["free"] < 30 or $wpmemory["percent"] > 85) { ?>
                <h2 style="color: red;">
                    <?php $message = esc_attr__("Low WordPress Memory Limit", 'cardealer'); ?>
                </h2>
                <?php
                $mb = "MB";
                echo "<b>";
                ...
                echo esc_attr__("Your usage now", 'cardealer') .
                    ": " .
                ...
                echo "|&nbsp;&nbsp;&nbsp;" .
                    esc_attr__("Total Php Server Memory", 'cardealer') .
                    " : " .
                ...
                echo '<p>';
                echo esc_attr__("Your WordPress Memory Limit is too low, which can lead to critical issues on your site due to insufficient resources. Promptly address this issue before continuing.", 'cardealer');
                echo '</b>';
                ?>
                </b>
                <a href="https://wpmemory.com/fix-low-memory-limit/">
                    <?php
                    echo esc_attr__("Learn More", 'cardealer');
                    ?>
                </a>
                </p>
                <br>
    ...
    <?php
                ...
                echo "<br />";
                echo esc_attr__("This is a partial list of the errors found.", 'cardealer');
                echo "<br />";
                /**
                ...
                function getFileSizeInBytes($bill_filename)
                {
                    if (!file_exists($bill_filename) || !is_readable($bill_filename)) {
                        ...
                        return esc_attr__("File not readable.", 'cardealer');
                    }
                    $fileSizeBytes = filesize($bill_filename);
                    if ($fileSizeBytes === false) {
                        //return "Size not determined.";
                        return esc_attr__("Size not determined.", 'cardealer');
                    }
                ...
                function convertToHumanReadableSize($sizeBytes)
                {
                    if (!is_int($sizeBytes) || $sizeBytes < 0) {
                        ...
                        return esc_attr__("Invalid size.", 'cardealer');
                    }
                    $units = ['B', 'KB', 'MB', 'GB', 'TB'];
                    $unitIndex = 0;
                ...
                foreach ($bill_folders as $bill_folder) {
                    ...
                    foreach ($files as $bill_filename) {
                        if (strpos($bill_filename, "backup") != true) {
                            ...
                            echo "<br />";
                            echo esc_attr__("File Size: ", 'cardealer');
        ...
        public function custom_help_tab()
        {
            ...
            if ("site-health" === $screen->id) {
                ...
                $screen->add_help_tab([
                    "id"      => "custom-help-tab",
                    "title"   => esc_attr__("Critical Issues", 'cardealer'),
                    "content" => "<p>" . $message . "</p>",
                ]);
            }
        }
